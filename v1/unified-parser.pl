...
# For use with various mailman instances.
#
# usage: mlparser 
#    --url=[archive url]
#
# What it should do:
#   0. Create the master sqlite DB if it doesn't exist.
#   1. Parse archive URL and pull out all relevant info.
#   2. Fetch recent month URLs (check DB).
#   3. Parse month URLs for individual post URLs.
#   4. Parse post URLs *only* if the URL is not already found in the DB.
#   5. Insert individual URL data into DB.
...
# 0.  Create the master sqlite db.  Schema as follows:
#   ml
...
$sqlite_result = `sqlite3 ml.db 'create table ml(msgid varchar(255),author_name varchar(255),author_email varchar(255), subject varchar(255), timedate varchar(255))'`;  # if table already exists, this will fail silently

# 1.  Pull out all important string data from the URL.
